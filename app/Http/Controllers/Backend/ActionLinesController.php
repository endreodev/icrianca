...
# --  
use App\Models\ActionLine as Model;
...
class ActionLinesController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->path         = 'action-lines';
        $this->route        = 'backend.frontend.action-lines';
    ...
    public function DataTable()
    {
        return Datatables::of($this->model::latest())
            ...
            ->addColumn('action', function ($row) {
                return View::make('backend.pages.' . $this->path . '.actions', ['row' => $row, 'route' => $this->route, 'path' => $this->path])->render();
            })
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('file-image')) {
                ...
                $size = [400, 400];
                $request->merge([
                    'image' => $this->upload($request,  $attr = 'file-image', $size)
                ]);
            }
            ...
            try {
                \LogActivity::addToLog('Usuário criou na sessão: ' . $this->model::NAME);
            } catch (\Exception $e) {
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $request->merge([
                ...
                'is_visible' => ($request->input('is_visible')) ? 1 : 2,
            ]);
            ...
            try {
                \LogActivity::addToLog('Usuário atualizou na sessão: ' . $this->model::NAME);
            } catch (\Exception $e) {
            }
