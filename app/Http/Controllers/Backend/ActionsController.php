...
class ActionsController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->path         = 'actions';
        $this->route        = 'backend.frontend.actions';
    ...
    {
        return Datatables::of($this->model::orderBy('id', 'DESC')->latest())
            // ->addIndexColumn()
            ...
            ->filterColumn('type', function ($query, $keyword) {
                ...
                $videos_parms   = ['videos', 'video', 'vídeos', 'vídeo'];
                $photos_parms   = ['fotos', 'galeria', 'galerias', 'foto'];
                $news_parms     = ['notícias', 'notícia', 'noticias', 'noticia'];

                if (in_array(strtolower($keyword), $videos_parms)) {
                    return $query->where('type', 'video');
                } else
                if (in_array(strtolower($keyword), $photos_parms)) {
                    return $query->where('type', 'photos');
                } else
                if (in_array(strtolower($keyword), $news_parms)) {
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->input('archives_upload')) {
                $archives_upload = $request->input('archives_upload');

                foreach ($archives_upload as $key => $archive) {
                    $item = Archive::where('id', $archive)->first();
                    $item->entity_id = $content->getId();
                    $item->entity = $content->type;
                    $item->save();
                }
            }
