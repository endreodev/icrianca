    ...
    public function DataTable()
    {
        return Datatables::of($this->model::query())
            ...
            ->addColumn('status', function ($row) {
                ...
                $status = '<span class="badge light badge-' . $row->getStatus()->badge . '"><i class="fa fa-circle text-' . $row->getStatus()->badge . ' mr-1"></i>' . $row->getStatus()->text . '</span>';
                $is_visible = (!$row->is_visible && $row->status) ? '<span class="badge light badge-' . $row->getIsVisible()->badge . '"><i class="fa fa-circle text-' . $row->getIsVisible()->badge . ' mr-1"></i>' . $row->getIsVisible()->text . '</span>' : '';
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (!$request->input('program_id')) {
                throw new \Exception('Você precisa selecionar pelo menos um programa');
            }
            ...
            $request->merge([
                'status' => ($request->input('status-radio') === "1") ? true : false,
            ]);
            $request->merge([
                'is_visible' => ($request->input('is_visible-radio') === "1") ? true : false,
            ]);
            ...
            $content->programs()->sync($request->input('program_id'));
