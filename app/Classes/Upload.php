...
trait Upload
{
    public function upload($file, $attr = 'image_upload', $size = ["1920", "1080", 1], $quality = 80)
    {
        ...
        sleep(3);
        $name = md5(time()) . md5(rand(2, 3)) . '.';

        $extension = $file->file($attr)->extension();
        ...
        $thumb = Image::make($file->file($attr))->resize(null, 400, function ($constraint) {
            ...
            $constraint->upsize();
        })->crop(400, 400)->stream($extension, 60);


        $resize = Image::make($file->file($attr))->resize(null, $size[1], function ($constraint) {
            $constraint->aspectRatio();
            $constraint->upsize();
        })->fit($size[0], $size[1])->stream($extension, 60);


        $original = Image::make($file->file($attr))->stream($extension, $quality);
    ...
    public function uploadFile($file, $attr = 'file', $path = 'files')
    {
        sleep(1);
        $name = md5(time()) . md5(rand(2, 3)) . '.';
        ...
        Storage::disk('public')->putFileAs($path, $file->file($attr), $name);
